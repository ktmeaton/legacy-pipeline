...
use Data::Dumper;
if (scalar(@ARGV) < 2) {
    print "Usage: prototype.pl input_fastq reference_genome(s)\n Redirect output to file called 'Makefile' and launch with command 'make'\nNote: reference genomes must be indexed using network aware bwa found at /home/ana/scripts/network-aware-bwa/bwa\n";
};
...
#    print Dumper(\%extramapped);
	...
	my $extraFilterFileName = $sampleName."_".$extraRefName.".min24.bam";
	...
	my $extraMinCmd = "$samtools view -b -m 24 -o $extraFilterFileName $extraMapFileName";
